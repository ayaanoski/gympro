...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Earnings', path: '/trainer-earnings', icon: Card, roles: ['trainer'] },
    { name: 'Clients', path: '/my-clients', icon: UserCheck, roles: ['trainer'] },
    { name: 'Profile', path: '/trainer-profile', icon: Settings, roles: ['trainer'] },
    ...
    { name: 'Profile', path: '/my-profile', icon: UserCheck, roles: ['member'] },
    { name: 'Attendance', path: '/my-attendance', icon: Calendar, roles: ['member'] },
  ];

  const filteredNavItems = navItems.filter(item =>
    item.roles.includes(userProfile?.role || '') || (isAdmin && item.roles.includes('admin'))
  );
      ...
      {isMember ? (
        <>
          ...
          <div className="bg-white/80 backdrop-blur-md border-b px-4 py-3 flex items-center justify-between sticky top-0 z-50">
            ...
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors md:hidden">
          ...
          `}>
            <div className="p-6 flex flex-col h-full">
                ...
                {filteredNavItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      to={item.path}
                      onClick={() => setIsSidebarOpen(false)}
                      className={`group flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${isActive ? 'bg-red-50/80 text-brand-primary font-bold' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900'}`}
                    >
              ...
              <div className="mt-auto pt-6 border-t border-gray-50">
                ...
                <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 text-red-500 hover:bg-red-50 rounded-xl transition-all duration-300 font-bold">
